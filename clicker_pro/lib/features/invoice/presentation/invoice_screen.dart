...
class InvoiceScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // On wide web the list view uses the dedicated desktop invoices dashboard
    // (KPI row + table). The single-invoice DETAIL view keeps the mobile layout
    // so its export/share actions stay intact. Mobile + narrow web unchanged.
    final webWide = inv == null &&
        kIsWeb &&
    ...
    if (webWide) {
      return Scaffold(
        ...
        body: WebInvoices(
          onTapInvoice: (i) => Navigator.of(context).push(
            MaterialPageRoute<void>(builder: (_) => InvoiceScreen(invoice: i)),
          ),
        ),
      );
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // Blank invoice — company header only, everything else left
          // empty so the owner can take a booking by hand on paper.
          if (inv == null)
            IconButton(
              tooltip: 'Blank invoice',
              icon: Icon(Icons.note_add_outlined, color: AppColors.gold),
              onPressed: () => _exportBlankInvoice(context, ref),
            ),
        ],
      ),
      body: inv == null
  ...
  /// Blank invoice — company details only (name, phone, address, logo,
  /// signature); the client/event/money fields are printed as labelled
  /// blank lines the owner fills in by hand when taking a manual booking.
  Future<void> _exportBlankInvoice(BuildContext context, WidgetRef ref) async {
    ...
    try {
      final logo = await PdfExporter.fetchImageBytes(user.logoUrl);
      final signature = await PdfExporter.fetchImageBytes(user.signatureUrl);
      const blank = '_______________________';
      await PdfExporter.share(
        PdfDocumentData(
          documentTitle: 'Invoice',
          fileName:
              'blank_invoice_${DateTime.now().millisecondsSinceEpoch}',
          companyName: user.companyName ?? user.name,
          companyPhone: user.phone ?? '',
          companyAddress: user.studioAddress ?? '',
          logoBytes: logo,
          ...
          detailRows: const [
            ...
            PdfRow('Time', blank),
            PdfRow('Venue', blank),
            PdfRow('Package', blank),
          ],
          summary: const [
            PdfRow('Total', blank),
            PdfRow('Advance', blank),
            PdfRow('Due', blank, emphasize: true),
          ],
